...
#(OCORRÊNCIAS DE HOMÍCIDIO DOLOSO + OCORRÊNCIAS DE LATROCÍNIO), apenas cidade de sp
...
crime_longer_doloso <- readr::read_csv("inputs_brutos/monthly_occurrences_policiaSP.csv") %>%
  ...
  dplyr::filter(Cidade == "São Paulo",
                Ano <= 2018,
                Natureza == "HOMICÍDIO DOLOSO (2)") %>% 
...
#juntar os datasets e fazer a soma para ter ocorrências de crimes letais na cidade de sp
...
#dataset wide com séries de produtividade policial
...
economia <- readxl::read_excel("inputs_brutos/rendimento_real_médio_dos_ocupados.xlsx", sheet = "Tab46") %>% 
